# @title using frozen hubert base model to extract the layer of embeddings for voxceleb 1 TEST dataset
import torch
import torchaudio
...
# === Settings ===
dataset_root = Path(r"C:\Datasets\voxceleb1\test_wav")   # test set location
output_root = Path("embedding_batches_test_zip")         # separate folder for test embeddings
...
print(f"Found {len(wav_files)} TEST WAV files to process.")
...
for idx, wav_path in enumerate(tqdm.tqdm(wav_files, desc="Extracting test embeddings")):
    try:
        # Output paths
        ...
        if file_in_batch == batch_size or idx == len(wav_files) - 1:
            ...
            print(f"\nZipping test batch {batch_count} to {zip_path}")
